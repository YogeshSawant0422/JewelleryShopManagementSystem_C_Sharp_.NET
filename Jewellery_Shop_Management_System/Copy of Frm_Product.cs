...
namespace Jewellery_Shop_Management_System
{
    public partial class Frm_Product_Details : Form
    {
        ...
        private void tb_Net_Wt_Leave(object sender, EventArgs e)
        {
            if (tb_Net_Wt.Text != "")
            {
                ...
                {
                    Price = Convert.ToDouble(Net_Wt) * Gold_Price / 10;

                    tb_Price.Text = Convert.ToString(Price);
                }
                else if (cmb_Category.Text == "Silver")
                {
                    Price = Convert.ToDouble(Net_Wt) * Silver_Price / 1000;
            ...
            {
                MessageBox.Show(" Please Do Not Leave With Null Value ??");
            }
        }
    }
}
